...
const ButtonBar: React.FC = () => {
  ...
  return (
    <div className="flex items-stretch justify-between gap-1 bg-gray-100 p-1 py-2">
      {/* Logo Section */}
      <div className="border-primary flex cursor-pointer items-center justify-center rounded-2xl border-2 bg-white px-5 py-5 shadow-lg shadow-gray-400 transition duration-150 ease-in-out hover:bg-gray-100">
        <Image
      ...
      <div className="flex flex-grow items-stretch justify-center gap-2 px-1">
        {buttonsData.map((button) => (
          <button
            ...
            className="border-secondary flex flex-grow cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 bg-[radial-gradient(circle,#74ACDA,_#25509A)] px-3 py-2 text-center text-white transition duration-150 ease-in-out hover:opacity-80"
            onClick={() => {
              router.push(buttonsLinks[button.label]);
            }}
          >
            <span className={`leading-tight font-medium ${styles.label}`}>
              {button.label}
            </span>
            <Image
              src={button.iconSrc}
      ...
      {/* Date/Time Section */}
      <div className="border-primary text-primary flex min-w-[100px] flex-col items-center justify-center rounded-xl border-2 bg-white px-4 py-2">
